...
class BaseDataset(torch.utils.data.Dataset):
  ...
  def get_source_img(self, idx, raw_img=False):
    return self.get_img(idx, raw_img=raw_img)

  def get_target_img(self, idx, raw_img=False):
    return self.get_img(idx, raw_img=raw_img)
...
class CSSDataset(BaseDataset):
  ...
  def __init__(self,
               ...
               transform=None,
               query_image_mode='3d',
               target_image_mode='3d'):
    super(CSSDataset, self).__init__()

    self.img_path = path + '/images/'
    self.transform = transform
    self.split = split
    self.query_image_mode = query_image_mode
    self.target_image_mode = target_image_mode
    self.data = np.load(
        path + '/css_toy_dataset_novel2_small.dup.npy',
        allow_pickle=True,
  ...
  def generate_random_query_target(self):
    ...
    out['source_img_data'] = self.get_source_img(img1id)
    out['target_img_id'] = img2id
    out['target_img_data'] = self.get_target_img(img2id)
    out['target_caption'] = self.imgs[img2id]['captions'][0]
  ...
  def get_source_img(self, idx, raw_img=False):
    return self.get_img(
        idx, raw_img=raw_img, get_2d=self.query_image_mode == '2d')

  def get_target_img(self, idx, raw_img=False):
    return self.get_img(
        idx, raw_img=raw_img, get_2d=self.target_image_mode == '2d')
