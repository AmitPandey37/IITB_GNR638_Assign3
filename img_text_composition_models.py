...
class ImgTextCompositionBase(torch.nn.Module):
  ...
  def compute_soft_triplet_loss_(self, mod_img1, img2):
    distances = torch_functions.pairwise_distances(mod_img1, img2)
    positives = distances.diag().unsqueeze(1)
    negatives = distances[~torch.eye(
        distances.shape[0], dtype=torch.bool, device=distances.device)]
    negatives = negatives.reshape(distances.shape[0], distances.shape[0] - 1)
  ...
  def compute_batch_based_classification_loss_(self, mod_img1, img2):
    ...
    labels = torch.arange(x.shape[0], device=x.device).long()
...
class ImgEncoderTextEncoderBase(ImgTextCompositionBase):
  ...
  def __init__(self, texts, embed_dim):
    ...
    if hasattr(torchvision.models, 'ResNet18_Weights'):
      ...
      img_model = torchvision.models.resnet18(weights=weights)
    else:
